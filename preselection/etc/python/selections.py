...
def selections(df):
    ...
    df = df.Filter("Flag_goodVertices && "
    ...
    df = df.Define("SC_absEta", "Electron_eta + Electron_deltaEtaSC") \
    ...
    df = df.Define("GLepton_pt", "isElectron ? GElectron_pt[0] : GMuon_pt[0]") \
    ...
    df = df.Define("HLepDeltaR", "fDeltaR(FatJet_eta, FatJet_phi, GLepton_eta, GLepton_phi)") \
    ...
    df = df.Define("WLepDeltaR", "fDeltaR(FatJet_eta, FatJet_phi, GLepton_eta, GLepton_phi)") \
    ...
    df = df.Define("AK4LepDeltaR", "fDeltaR(Jet_eta, Jet_phi, GLepton_eta, GLepton_phi)") \
    ...
    df = df.Define("ST", "GLepton_pt + GHiggs_pt + GW_pt + MET_pt") \
